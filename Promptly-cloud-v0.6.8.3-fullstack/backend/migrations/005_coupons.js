...
// =============================================
const existing = db.prepare("SELECT id FROM coupons WHERE code = ?").get("PROMPTLY-DEE1636310A6");
if (!existing) {
  db.prepare(`
    INSERT INTO coupons (code, plan, max_redemptions, duration_days, active)
    VALUES (?, 'monthly', 10, 30, 1)
  `).run("PROMPTLY-DEE1636310A6");
  console.log("[migration-005] ✓ Seeded friends & family coupon: PROMPTLY-DEE1636310A6");
} else {
