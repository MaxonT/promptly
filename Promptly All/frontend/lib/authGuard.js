(function () {
  const API_BASE =
    (window.PROMPTLY_API_BASE && window.PROMPTLY_API_BASE.trim()) ||
  ...
  async function syncTimezone() {
    ...
    const lastAttemptAt = Number(localStorage.getItem("promptly.tz.sync_at") || "0");
    if (Number.isFinite(lastAttemptAt) && Date.now() - lastAttemptAt < 12 * 60 * 60 * 1000) {
      return;
    }
    localStorage.setItem("promptly.tz.sync_at", String(Date.now()));
    ...
    try {
      ...
      if (res.ok && data.timezone) {
        localStorage.setItem("promptly.timezone", data.timezone);
      }
    } catch {
    }
  }
