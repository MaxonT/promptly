...
/**
 * Validates and enhances a single question with proper options
 * Extracted for readability and modularity
 * @param {Object} q - Question object to validate
 * @param {number} index - Question index for ID generation
 * @returns {Object} Validated and enhanced question
 */
function validateAndEnhanceQuestion(q, index) {
...
/**
 * Adds default options based on question content
 * @param {Object} q - Question object to enhance
 */
function addDefaultOptions(q) {
  if (q.depth_enabled) {
...
/**
 * Ensures "Other" option exists in all option arrays
 * @param {Object} q - Question object to check
 */
function ensureOtherOption(q) {
...
export async function generateChoiceQuestions({ initialDescription, kind, broadQuestions }) {
  ...
  const validatedQuestions = parsed.choice_questions.map((q, index) => validateAndEnhanceQuestion(q, index));
