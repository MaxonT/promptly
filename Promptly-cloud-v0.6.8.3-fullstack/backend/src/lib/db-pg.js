...
const getDatabaseConfig = () => {
  ...
  return {
    ...
    database: process.env.DB_NAME || 'promptly',
...
// Test connection
pool.on('error', (err) => {
  console.error('[promptly] Unexpected PostgreSQL pool error:', err);
});
...
export const db = {
  ...
  async query(sql, params = []) {
    try {
      ...
      console.error('[promptly] PostgreSQL query error:', err);
      throw err;
    }
  },
...
export async function ensureDemoUser() {
  try {
    await db.run(`
      ...
      VALUES ('demo-user', 'demo@promptly.local', CURRENT_TIMESTAMP)
