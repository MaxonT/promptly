 ...
 * 
 * 使用方式:
 *   cd backend && node ../scripts/sync-data-to-cloud.js <cloud-url>
 *   例如: cd backend && node ../scripts/sync-data-to-cloud.js https://promptly-v0-6-cloudtest-cursor-dev.onrender.com
 * 
 * 原理:
 *   1. 读取本地SQLite数据库
 *   2. 通过API接口将数据导入到云端
...
const dbPath = path.resolve(__dirname, '../backend/data/app.db');

const cloudUrl = process.argv[2];
if (!cloudUrl) {
  console.error('❌ 请提供云端URL');
  console.error('使用方式: node scripts/sync-data-to-cloud.js <cloud-url>');
  console.error('例如: node scripts/sync-data-to-cloud.js https://promptly-v0-6-cloudtest-cursor-dev.onrender.com');
  process.exit(1);
}
...
async function syncData() {
  try {
    ...
    const analyticsUsers = db.prepare('SELECT * FROM analytics_users LIMIT 100').all();
    const analyticsSessions = db.prepare('SELECT * FROM analytics_sessions LIMIT 100').all();
    const analyticsBehavior = db.prepare('SELECT * FROM analytics_behavior LIMIT 100').all();
    ...
    console.log(`   📊 同步统计:`);
    console.log(`      - Analytics数据行: ${result.analyticsCount || analyticsUsers.length + analyticsSessions.length}`);
    console.log(`      - Pipeline数据行: ${result.pipelineCount || specs.length + runs.length}`);
    ...
    console.error('');
    console.error('💡 可能的原因:');
    console.error('   1. 云端URL不正确');
    console.error('   2. 云端服务未启动');
    console.error('   3. 云端不支持 /api/admin/sync-data 端点');
    console.error('');
    process.exit(1);
  }
}
