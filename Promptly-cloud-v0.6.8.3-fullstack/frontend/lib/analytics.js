...
function getSessionId() {
  if (!sessionId) {
    sessionId = localStorage.getItem('promptly.analytics_session') || generateId();
    localStorage.setItem('promptly.analytics_session', sessionId);
  }
  return sessionId;
}
...
export function track(event, properties = {}) {
  ...
  // 同时输出到控制台（开发环境）
  if (window.PROMPTLY_DEBUG || localStorage.getItem('promptly.debug') === '1') {
