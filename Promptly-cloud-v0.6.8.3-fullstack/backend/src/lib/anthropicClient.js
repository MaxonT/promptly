...
if (apiKey) {
  ...
  console.log(`[promptly] ✅ Anthropic client initialized successfully`);
  const maskedKey = apiKey.length > 11
    ? `${apiKey.substring(0, 7)}...${apiKey.substring(apiKey.length - 4)}`
    : "***";
  console.log(`[promptly] Anthropic API Key: ${maskedKey} (masked)`);
} else {
  console.warn("[promptly] ⚠️  ANTHROPIC_API_KEY is not set; Anthropic features are disabled.");
}
...
export async function chatJsonAnthropic({ system, user, model, apiKey: overrideKey, maxTokens, temperature }) {
  ...
  if (!client) {
    console.error("[promptly] ❌ Anthropic call blocked: client not initialized (ANTHROPIC_API_KEY not set)");
  ...
  try {
    ...
    console.log(`[promptly] ✅ Anthropic call succeeded - Duration: ${duration}ms, Response: ${content.length} chars, Tokens: ${totalTokens} (in:${inputTokens} out:${outputTokens})`);
    ...
    try {
      ...
      try {
        ...
        console.error(`[promptly] ⚠️  Anthropic JSON parse failed:`, parseError.message);
        parsed = {};
      }
    }
    ...
    console.error(`[promptly] ❌ Anthropic call failed:`, error.message);
    throw error;
  }
}
...
export async function chatTextAnthropic({ system, user, model, apiKey: overrideKey, maxTokens, temperature, minSimilarity, maxRetries }) {
  ...
  try {
    ...
    if (minSimilarity && maxRetries && maxRetries > 0) {
      console.log(`[promptly] ⚠️ Similarity check not implemented for Anthropic (returns 0). minSimilarity=${minSimilarity}, maxRetries=${maxRetries}`);
    }
